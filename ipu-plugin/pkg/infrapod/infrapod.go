...
func CreateInfrapod() error {
	...
	/*
		// Create p4 pv
		err = render.ApplyAllFromBinData(log, "p4-pv", nil, binData, mgr.GetClient(), nil, mgr.GetScheme())
		if err != nil {
			return fmt.Errorf("failed to start p4-pv (p4Image): %v", err)
		}
		// Create p4 pvc
		err = render.ApplyAllFromBinData(log, "p4-pvc", nil, binData, mgr.GetClient(), nil, mgr.GetScheme())
		if err != nil {
			return fmt.Errorf("failed to start p4-pvc (p4Image): %v", err)
		}
	*/
	// Create p4 pod
