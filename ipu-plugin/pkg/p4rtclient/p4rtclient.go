...
func programFXPP4Rules(p4RtBin string, ruleSets []fxpRuleBuilder) error {
		...
		p4rule := []string{fmt.Sprintf("Action: %s, P4br: %s, Control: %s, Metadata: %s",
	                   r.Action, r.P4br, r.Control, r.Metadata)}
                if err := utils.RunP4rtCtlCommand(p4RtBin, p4rule...); err != nil {
                        return err
                }
        }
	return nil
