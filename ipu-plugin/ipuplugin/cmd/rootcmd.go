...
func getBridgeController(bridgeName, bridgeType, ovsCliDir string) (types.BridgeController, types.BridgeType) {
	switch bridgeType {
	case "ovs":
		return ipuplugin.NewOvsBridgeController(bridgeName, ovsCliDir), types.OvsBridge
	case "linux":
