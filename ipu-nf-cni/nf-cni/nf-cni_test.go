...
type IpuManagerServer struct {
	grpcServer *grpc.Server
	address    string
	pb.UnimplementedIpuManagerServiceServer
}

func (s *IpuManagerServer) CreateIpuManagerRequest(ctx context.Context, in *pb.IpuManagerRequest) (*pb.IpuManagerResponse, error) {
	response := "Received"
	return &pb.IpuManagerResponse{Message: &response}, nil
}

func NewIpuManagerServer() *IpuManagerServer {
	return &IpuManagerServer{grpcServer: grpc.NewServer(), address: "localhost:50551"}
}

func (s *IpuManagerServer) Start() error {
	lis, err := net.Listen("tcp", s.address)
	if err != nil {
		return fmt.Errorf("error starting the Ipu Manager server: %v", err)
	}

	go func() {
		err := s.grpcServer.Serve(lis)
		if err != nil {
			fmt.Errorf("serving incoming requests failed: %v", err)
		}
	}()
	...
	if err != nil {
		return fmt.Errorf("error. unable to establish test connection with %s gRPC server: %v", s.address, err)
	}
	conn.Close()
...
func (s *IpuManagerServer) Stop() error {
	if s.grpcServer == nil {
		return nil
	}
	s.grpcServer.Stop()
...
var server *IpuManagerServer

var _ = BeforeSuite(func() {
	server = NewIpuManagerServer()
	pb.RegisterIpuManagerServiceServer(server.grpcServer, server)
	server.Start()
})
...
var _ = Describe("base functionality", func() {
	...
	for _, ver := range []string{"0.3.0", "0.3.1", "0.4.0", "1.0.0"} {
		...
		It(fmt.Sprintf("[%s] fails an invalid config", ver), func() {
			...
			Expect(err).To(MatchError(`specify "deviceID"`))
		})
		...
		It(fmt.Sprintf("[%s] works with a valid config without IPAM", ver), func() {
			...
			if testutils.SpecVersionHasCHECK(ver) {
				...
				// CNI Check nf-cni in the target namespace
